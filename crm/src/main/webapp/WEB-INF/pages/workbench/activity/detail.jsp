		...
		<c:forEach items="${remarkList}" var="remark">
			...
			<div style="position: relative; top: -40px; left: 40px;" >
				...
				<small style="color: gray;">
                    ...
                    <%-- 写法2 --%>
                    <%--<c:if test="${remark.editFlag=='1'}"></c:if> ${remark.editTime}
                    <c:if test="${remark.editFlag!='1'}"></c:if> ${remark.createTime}--%>
                    <%-- 创建者还是修改者 --%>
					${remark.editFlag=='1'?remark.editBy:remark.createBy}
                    <%-- 创建还是修改 --%>
					${remark.editFlag=='1'?'创建':'修改'}
				</small>
				<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">
                        <%-- 自定义remarkId属性，用于保存备注的id --%>
					<a remarkId="${remark.id}" class="myHref" href="javascript:void(0);">
